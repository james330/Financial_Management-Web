...
static  void  TaskStartDisp(void);
		...
		void InsertionSort(INT8U *arr, INT8U size);
...
static void TaskStartCreateTasks(void)
{
	INT8U i;
	char s[80];
	MyStartTime=OSTimeGet();//Catch OS Start time as zero 
...
{	
	...
	for(;;)
	{
		x++;myT++;
		sprintf(s, "%4d",x);
		PC_DispStr(75, 10 + OSPrioCur - PERIODIC_TASK_START_PRIO, s, DISP_FGND_BLACK + DISP_BGND_LIGHT_GRAY);//run
		sprintf(s, "%10d ",OSTimeGet() );
		PC_DispStr(15, 10+OSPrioCur - PERIODIC_TASK_START_PRIO, s, DISP_FGND_BLACK + DISP_BGND_LIGHT_GRAY);//start time
		sprintf(s,"%10d ", MyPtr->Deadline);
		PC_DispStr(39, 10 + OSPrioCur - PERIODIC_TASK_START_PRIO, s, DISP_FGND_BLACK+DISP_BGND_LIGHT_GRAY);//dead line
		
		sprintf(s,"%3d",TaskNum[OSPrioCur-PERIODIC_TASK_START_PRIO]);
		PC_DispStr(0+3*myT,19,s,DISP_FGND_BLACK+DISP_BGND_LIGHT_GRAY);
		sprintf(s,"%3d",OSTimeGet()/OS_TICKS_PER_SEC);
		PC_DispStr(0+3*myT,20,s,DISP_FGND_BLACK+DISP_BGND_LIGHT_GRAY);
		...
		sprintf(s, "%10d ",OSTimeGet() );
		PC_DispStr(27, 10 + OSPrioCur - PERIODIC_TASK_START_PRIO, s, DISP_FGND_BLACK+DISP_BGND_LIGHT_GRAY);//end time

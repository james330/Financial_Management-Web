...
TASK_EXTRA_DATA *MyPtr;
	...
	if((OSPrioCur>=PERIODIC_TASK_START_PRIO)&&(OSPrioCur<=PERIODIC_TASK_START_PRIO+7)){//check running task is periodic task or not
		MyPtr=OSTCBCur->OSTCBExtPtr;
		if(MyPtr->RemainTime>0){MyPtr->RemainTime--;}//-- until test.c <=0 break , then sleep until next period
	}
    if (OSRunning == TRUE) {    
